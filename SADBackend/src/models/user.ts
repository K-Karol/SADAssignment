...
export function GenerateBaseExcludes(rootPath: string, exception: string[] = []) {
  var passwordObj: any = {};
  passwordObj[rootPath + "password"] = 0;
  if (!exception.includes("roles"))
    passwordObj[rootPath + "roles"] = 0;
  if (!exception.includes("address"))
    passwordObj[rootPath + "address"] = 0;
  return [passwordObj]
}

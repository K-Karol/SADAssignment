...
import { GetSessionForStudentBody, GetSessionForStudentParams, GetAttendenceForSessionParams, GetAttendenceForStudentParams_ValidationStage, GetSessionsQuery, SessionPostRequest } from '../validation/session';
export default class SessionRoute implements IRoute {
  ...
  private initializeRoutes() {
    ...
    this.router.get(`${this.path}GetUserAttendence/:sessionID/:studentID`, AuthenticateRequest, ValidationMiddleware(GetAttendenceForStudentParams_ValidationStage, 'params'), ValidationMiddleware(GetSessionsQuery, 'query'), ValidationMiddleware(GetSessionForStudentBody, 'body'), this.sessionController.GetUserAttendence);

...
import { GetModuleByID_ValidationStage, GetModulesQuery, ModulePutRequest_ValidationStage, PostModuleRequest_ValidationStage } from '../validation/module';
export default class ModuleRoute implements IRoute {
  ...
  private initializeRoutes() {
    ...
    this.router.get(`${this.path}resource/`, AuthenticateRequest, AuthoriseByRoles(["Admin", "Staff"]), ValidationMiddleware(GetModulesQuery, 'query'),  this.moduleController.GetModules);
    this.router.get(`${this.path}resource/:id`, AuthenticateRequest, AuthoriseByRoles(["Admin"]), ValidationMiddleware(GetModuleByID_ValidationStage, 'params'),  this.moduleController.GetModule);
